# Решение задачи 8

---

## Часть 1: 7 независимых заданий и 5 исполнителей

**Дано:**
- Длительности заданий: $5,\ 5,\ 6,\ 7,\ 6,\ 11,\ 15$
- Количество исполнителей: $5$
- Прерывания задач разрешены.

**Решение:**
1. Найдем максимальную длительность задания:

   $$
   T_{\text{max}} = \max(5,\ 5,\ 6,\ 7,\ 6,\ 11,\ 15) = 15
   $$

3. Вычислим среднюю нагрузку на одного исполнителя:

   $$
   T_{\text{avg}} = \frac{5 + 5 + 6 + 7 + 6 + 11 + 15}{5} = \frac{55}{5} = 11
   $$

5. Оптимальное время выполнения определяется как:

   $$
   T_{\text{opt}} = \max(T_{\text{max}},\ T_{\text{avg}}) = \max(15,\ 11) = 15
   $$

**Ответ:**  
**15**

```mermaid
gantt
    title Диаграмма Ганта - Ленточная стратегия для 5 исполнителей
    dateFormat  HH:mm    
    axisFormat %H:%M
    Начало выполнения работ : milestone, m1, 00:00, 0h
section Исполнитель 1
    Задача A (5) :a1, 00:00, 5h
    Задача B (5) :b1, 05:00, 5h
    Задача C (5) :d1, 10:00, 5h
section Исполнитель 2
    Задача C (1) :d1, 00:00, 1h
    Задача D (7) :c1, 01:00, 7h
    Задача E (6) :c2, 08:00, 6h
    Задача F (1) :d1, 14:00, 1h
section Исполнитель 3
    Задача F (10) :d1, 00:00, 10h
    Задача G (5) :d1, 10:00, 5h
section Исполнитель 4
    Задача G (10) :d1, 00:00, 10h
    Простой :d2, after d1, 5h
    section Исполнитель 5
    Окончание выполнения работ : milestone, m2, 15:00, 0h
```

---

## Часть 2: Конвейерная задача с 10 заданиями и 2 исполнителями

**Дано:**
- Длительности этапов заданий:  
  $(4,7),\ (4,5),\ (8,6),\ (6,8),\ (5,5),\ (6,4),\ (2,4),\ (6,3),\ (4,9),\ (5,8)$
- Исполнитель 1 выполняет первый этап ($a_i$), Исполнитель 2 — второй этап ($b_i$).

**Решение (алгоритм Джонсона):**

1. **Разделение на группы:**
   - **Группа 1** ($a_i \leq b_i$):  
     $(2,4),\ (4,5),\ (4,7),\ (4,9),\ (5,5),\ (5,8),\ (6,8)$
   - **Группа 2** ($a_i > b_i$):  
     $(8,6),\ (6,4),\ (6,3)$

2. **Сортировка:**
   - Группа 1 отсортирована по возрастанию $a_i$.
   - Группа 2 отсортирована по убыванию $b_i$.

3. **Общий порядок выполнения:**  
   $(2,4),\ (4,5),\ (4,7),\ (4,9),\ (5,5),\ (5,8),\ (6,8),\ (8,6),\ (6,4),\ (6,3)$

4. **Расчёт времени завершения:**

   | Задание | $a_i$ | $b_i$ | $C_1$ (время окончания этапа 1) | $C_2$ (время окончания этапа 2) |
   |---------|--------|--------|----------------------------------|----------------------------------|
   | 1 | 2 | 4 | 2 | $2 + 4 = 6$ |
   | 2 | 4 | 5 | $2 + 4 = 6$ | $\max(6, 6) + 5 = 11$ |
   | 3 | 4 | 7 | $6 + 4 = 10$ | $\max(10, 11) + 7 = 18$ |
   | 4 | 4 | 9 | $10 + 4 = 14$ | $\max(14, 18) + 9 = 27$ |
   | 5 | 5 | 5 | $14 + 5 = 19$ | $\max(19, 27) + 5 = 32$ |
   | 6 | 5 | 8 | $19 + 5 = 24$ | $\max(24, 32) + 8 = 40$ |
   | 7 | 6 | 8 | $24 + 6 = 30$ | $\max(30, 40) + 8 = 48$ |
   | 8 | 8 | 6 | $30 + 8 = 38$ | $\max(38, 48) + 6 = 54$ |
   | 9 | 6 | 4 | $38 + 6 = 44$ | $\max(44, 54) + 4 = 58$ |
   | 10| 6 | 3 | $44 + 6 = 50$ | $\max(50, 58) + 3 = \textbf{61}$|

**Ответ:**  
**61**

```mermaid
gantt
    title Диаграмма Ганта - Конвейерная задача
    dateFormat  HH:mm
    axisFormat  %H:%M
section Начало
    Начало выполнения работ : milestone, m1, 01 00:00, 0h
section Исполнитель 1 (этап 1)
    A :a1, 01 00:00, 2h
    B :a2, after a1, 4h
    C :a3, after a2, 4h
    D :a4, after a3, 4h
    E :a5, after a4, 5h
    F :a6, after a5, 5h
    G :a7, after a6, 6h
    H :a8, after a7, 8h
    I :a9, after a8, 6h
    J :a10, after a9, 6h
section Исполнитель 2 (этап 2)
    A :b1, after a1, 4h
    B :b2, after a2 b1, 5h
    C :b3, after a3 b2, 7h
    D :b4, after a4 b3, 9h
    E :b5, after a5 b4, 5h
    F :b6, after a6 b5, 8h
    G :b7, after a7 b6, 8h
    H :b8, after a8 b7, 6h
    I :b9, after a9 b8, 4h
    J :b10, after a10 b9, 3h
section Окончание
    Окончание выполнения работ : milestone, m2, after b10, 0h
```
