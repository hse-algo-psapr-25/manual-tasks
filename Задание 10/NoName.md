### Вариант 8:
#### Количество работников: 2
#### Таблица зависимостей заданий:

| **Предшествующее** | **Последующее** |
|--------------------|-----------------|
| A                  | B               |
| B                  | D               |
| A                  | D               |
| B                  | I               |
| D                  | I               |
| A                  | C               |
| C                  | E               |
| E                  | H               |
| H                  | K               |
| B                  | F               |
| F                  | G               |
| G                  | L               |
| I                  | J               |
| K                  | J               |
| L                  | M               |
| J                  | M               |
| H                  | N               |
| M                  | O               |
| N                  | O               |



## Изначальный граф

```mermaid
graph TB

A --> B
B --> D
A --> D
B --> I
D --> I
A --> C
C --> E
E --> H
H --> K
B --> F
F --> G
G --> L
I --> J
K --> J
L --> M
J --> M
H --> N
M --> O
N --> O
  ```
Количество вершин = 15. Количество ребер = 19.

## Выбор стратегии.
Получили граф - лес, где О является стоком. Так как полученный граф - лес, направленный к корню О, нужно применять уровневую стратегию.


## Расстановка приоритетов
Сток графа — O.

Для построения расписания необходимо назначить приоритет для каждой задачи. В первую очередь приоритеты 1, 2, 3, ... назначаются стокам графа.
Далее поднимаемся вверх,и выбираем вершину, у которой все потомки уже имеют приоритеты.После формируем строку приоритетов 

O — 1
M — 2
N — 3
L — 4
J — 5
G — 6
K — 7
I — 8
F — 9
H — 10
D — 11
E — 12
B — 13
C — 14
A — 15

## Итоговый граф с приоритетами
```mermaid
graph TB
  A((A #15 <14,13>))-->B((B #13 <11,9>))
  A-->C((C #14 <12>))
  B-->D((D #9 <6>))
  B-->F((F #11 <8>))
  D-->I((I #6 <4>))
  C-->E((E #12 <10>))
  E-->H((H #10 <7,3>))
  H-->K((K #7 <4>))
  H-->N((N #3 <1>))
  K-->J((J #4 <2>))
  I-->J
  J-->M((M #2 <1>))
  F-->G((G #8 <5>))
  G-->L((L #5 <2>))
  L-->M
  M-->O((O #1 <>))
  N-->O
```

## Построение расписания для 2 исполнителей
Выбираем задачи, у которых выполнены все предшественники в каждый момент времени ,
и запускаем две задачи с максимальными приоритетами


0–1: A(15), C(14)
1–2: B(13), E(12)
2–3: D(11), H(10)
3–4: F(9), I(8)
4–5: K(7), G(6)
5–6: J(5), L(4)
6–7: N(3), M(2)
7–8: O(1)

## Диаграмма Ганта
```mermaid
gantt
  title Диаграмма Ганта
  dateFormat HH:mm
  axisFormat %H:%M

  Начало : milestone, m1, 00:00, 0h

  section Исполнитель 1
  A(15) :a, 00:00, 1h
  B(13)  :b, 01:00, 1h
  D(11)  :d, 02:00, 1h
  F(9)  :f, 03:00, 1h
  K(7)  :k, 04:00, 1h
  J(5)  :j, 05:00, 1h
  N(3) :n, 06:00, 1h
  O(1) :o, 07:00, 1h

  section Исполнитель 2
  C(14) :c, 00:00, 1h
  E(12)  :e, 01:00, 1h
  H(10)  :h, 02:00, 1h
  I(8)  :i, 03:00, 1h
  G(6)  :g, 04:00, 1h
  L(4) :l, 05:00, 1h
  M(2) :m, 06:00, 1h

  Конец : milestone, m2, 08:00, 0h
```
**Ответ: Длительность оптимального расписания составлет 8 условных единиц времени.**
