...
5. Построим чередующуюся цепь из непокрытой вершины $C$.
...
Получили цепь из непокрытой вершины $C$ в непокрытую вершину 4. Теперь "перекрасим" цепь - в выбранное паросочетание добавим рёбра $[C, 5]$ и $[E, 4]$, а ребро $[E, 5]$ из него уберём. Отобразим новое паросочетание на графе:
...
Непокрытыми остались вершины $B$ и 2.

6. Начнём строить цепь из вершины $B$.
...
7. Повторно выполним редукцию матрицы затрат. Выпишем множества: $X = \\{ B, C\\}$,  $Y = \\{ 5\\}$. Минимальный элемент необходимо искать в строках матрицы $B$ и $C$ и столбцах 1, 2, 3, 4. Это элемент 1, вычтем его из указанных строк и прибавим к столбцу 5, чтобы при вычитании из нулей не получить отрицательные значения.
   |       | **1** | **2** | **3** | **4** | **5** | | 
   |-------|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|
   | **A** |   2   |  2    |  **0**    |   **0**   |    2   |  |
   | **B** |  3    |  3    |  1   |  1    |   **0**   |  _-1_  |
...
Новые нули появились в ячейках $[B, 3]$, $[B. 4]$, $[C, 3]$, $[C, 4]$, добавим соответствующие им рёбра на граф.
...
9. Построим чередующуюся цепь из непокрытой вершины $B$.
...
Получили цепь из непокрытой вершины $B$ в непокрытую вершину 2. "Перекрасим" её: в паросочетание добавим рёбра $[B, 4]$, $[E, 1]$, $[D, 2]$ и уберём из него $[E, 4]$ и $[D, 1]$. Имеем **совершенное** паросочетание: $[A, 3]$, $[B, 4]$, $[C, 5]$, $[D, 2]$, $[E, 1]$.
...
10. Минимальные стоимости определим по ячейкам матрицы затрат, соответствующим совершенному паросочетанию:
   ...
   Общая стоимость затрат: 11 + 10 + 5 + 7 + 8 = 41.
